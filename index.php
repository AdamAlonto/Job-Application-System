...
function createApplicant($first_name, $last_name, $email, $phone_number, $address, $experience_years, $specialization) {
    ...
    $stmt = $pdo->prepare("INSERT INTO Applicants (first_name, last_name, email, phone_number, address, experience_years, specialization) 
                           VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$first_name, $last_name, $email, $phone_number, $address, $experience_years, $specialization]);
...
function updateApplicant($applicant_id, $first_name, $last_name, $email, $phone_number, $address, $experience_years, $specialization) {
    ...
    $stmt = $pdo->prepare("UPDATE Applicants 
                           SET first_name = ?, last_name = ?, email = ?, phone_number = ?, address = ?, experience_years = ?, specialization = ? 
                           WHERE applicant_id = ?");
    $stmt->execute([$first_name, $last_name, $email, $phone_number, $address, $experience_years, $specialization, $applicant_id]);
...
<body>
            ...
            <?php else: ?>
                <?php foreach ($search_results as $applicant): ?>
                    <tr>
                        ...
                        <td>
                            <a href="?delete=<?php echo $applicant['applicant_id']; ?>">Delete</a> |
    ...
    <?php if (isset($response)): ?>
        <div class="response <?php echo isset($response['statusCode']) && $response['statusCode'] === 200 ? '' : 'error'; ?>">
